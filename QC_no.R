...
data.n <- as.matrix(data.u)
col.names <- colnames(data.n)
...
ethnic <- cbind(ethnic, data.n[,ethnicity])
...
to_delete <- by_ethnicity
length(to_delete)

# 1 MISSINGNESS
miss <- grep("22005", col.names)
missing <- as.matrix(no.ind)
missing <- cbind(missing, data.n[,miss])
by_miss <- c()
...
for (i in chr) {
  ...
  genetic <- cbind(genetic,data.n[,gen])
...
# for (i in pc) {
#   data.1 <- cbind(data.1, as.in(data.n[,i]))
# }
...
# 3 RELATED
related <- read.table("/hpc/hers_en/mbombari/ukbb/ukb55392_rel_s488265.dat", header = T)
r <- as.matrix(no.ind)
colnames(r)[1] <- "ID1"
...
individuals_20002 <- matrix(nrow = nrow(data.n),ncol = 0)
individuals_20002 <- cbind(data.n[,2], individuals_20002)
for (i in id20002){
  individuals_20002 <- cbind(individuals_20002, data.n[,i])
}
...
individuals_20544 <- matrix(nrow = nrow(data.n),ncol = 0)
individuals_20544 <- cbind(data.n[,2], individuals_20544)
for (i in id20544){
  individuals_20544 <- cbind(individuals_20544, data.n[,i])
}
...
individuals_41202 <- matrix(nrow = nrow(data.n),ncol = 0)
individuals_41202 <- cbind(data.n[,2], individuals_41202)
for (i in id41202){
  individuals_41202 <- cbind(individuals_41202, data.n[,i])
}

id41204 <- grep("41204",col.names)
individuals_41204 <- matrix(nrow = nrow(data.n),ncol = 0)
individuals_41204 <- cbind(data.n[,2], individuals_41204)
for (i in id41204){
  individuals_41204 <- cbind(individuals_41204, data.n[,i])
}

id40001 <- grep("40001",col.names)
individuals_40001 <- matrix(nrow = nrow(data.n),ncol = 0)
individuals_40001 <- cbind(data.n[,2], individuals_40001)
for (i in id40001){
  individuals_40001 <- cbind(individuals_40001, data.n[,i])
}
...
n.ind <- no.ind[!(no.ind %in% to_delete)]
n.ind <- cbind(n.ind,n.ind)
write.table(n.ind, file = "/hpc/hers_en/psobrevals/bombari/no_ind_QC.txt", row.names = F)
